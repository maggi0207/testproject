 const returnOptions = () => {
    ...
    let dropOffStoreText = isEnabledTradeReturnOptionsInContactCenterApps ? "Bring device to a Verizon store" : "Drop device off at Verizon store later";
    //console.log('getShippingItemDetails', this.getShippingItemDetails());
    ...
    let shippingOptions = [
      {
        ...
        children:
          <div className="Grid shippingOption u-paddingNone">
            ...
            <div className="Row optionSubText">{"The trade-in device can be assessed and collected for processing in the store"}</div>
          </div>,
        value: "tradeinNow",
          ...
          <>
            ...
            <TextLinkCaretWrapper data-testid='editLocationWrapper'>
              <TextLinkCaret
                ...
                onClick={() => chooseTradeInSelected === 'tradeinLater' && setCurrentSection('StoreSelection')}
              >
                Edit location
              </TextLinkCaret>
      ...
      const shippingLabelandQrOption = [
        {
          ...
          children:
            <div className="Grid shippingOption u-paddingNone">
              ...
              <div className="Row optionSubText">{"We'll email you a prepaid shipping label. Print it, attach it to your own box and bring it to a UPS drop off location."}</div>
            </div>,
          value: "PrepaidLabel",
        ...
        },
        {
          ...
          children:
            <div className="Grid shippingOption u-paddingNone">
              ...
              <div className="Row optionSubText">{"We'll email you a QR code. Show this code at the UPS Store and they'll package and ship your device at no cost."}</div>
            </div>,
    ...
    if (getChannel() && getChannel().includes("OMNI-RETAIL") && isIspuItemInCart && customerType?.toLowerCase() !== "n") {
      const ispuOption = {
        name: "group",
        children:
          <div className="Grid shippingOption u-paddingNone">
            <div className="bold optionText">{"Use the same return location as your In-store pickup location"}</div>
    ...
    if (preOrBackOrder || isIspuItemInCart || isEnabledTradeReturnOptionsInContactCenterApps) {
      shippingOptions = shippingOptions.filter(
        option => (option.value != "tradeinNow")
      )
    }
    if (isEnabledTradeReturnOptionsInContactCenterApps && isMultiTrade) {
      shippingOptions = shippingOptions.filter(
        option => (option.value != "UPSDrop-off_QRCode")
      )
    }
